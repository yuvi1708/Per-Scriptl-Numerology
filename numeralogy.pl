...
if($numeralogy == 41)
{
	my $file = 'C:\1.Perl\selected_names.txt';

	open(FR,"<$file") or die "Can not open file $file : $!\n";
	my $file_content = do{local $/; <FR>;};
	close FR;
		
	open(FW,"+>>$file") or die "Can not open file $file : $!\n";

	if($file_content !~ m/$name_full/)
	{
		print FW $name_full."\n\n";
	}
	else
	{
		print "The name $name_full already exists in the file \n\n";
	}
